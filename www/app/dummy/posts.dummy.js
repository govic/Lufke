var dummyPosts = [
	{
		"postId": 1,
		"backgroundImgUrl": "http://goo.gl/56Q265",
		"profile": {
			"image": "http://goo.gl/PXFWyj",
			"name": "Soila Cuevas"
		},
		"location": "Pichilemu",
		"timestamp": "1396621153000",
		"text": "Tengo lindos ojos vieron?",
		"totalStars": 42,
		"totalComments": 3,
		"link": "",
		"comments": [
			{
				"commentId": "54dcdc1cabff9284fc32e01a",
				"profile": {
					"name": "Mullins Dominguez",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Enim incididunt veniam dolore officia laboris nostrud."
			},
			{
				"commentId": "54dcdc1cad07d034ae2448cc",
				"profile": {
					"name": "Estes Frazier",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Ex ut aliqua ullamco sint."
			},
			{
				"commentId": "54dcdc1c88638591ea145947",
				"profile": {
					"name": "Dean Ward",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Eu ipsum veniam officia non deserunt est dolore Lorem labore."
			}
		]
	},
	{
		"postId": 2,
		"backgroundImgUrl": "http://i.imgur.com/xxyIUPy.jpg",
		"profile": {
			"image": "http://goo.gl/9kbu0C",
			"name": "Elsa Popeludo"
		},
		"location": "Viña del Mar",
		"timestamp": "1396621153000",
		"text": "Hola mundo",
		"totalStars": 2,
		"totalComments": 5,
		"link": "",
		"comments": [
			{
				"commentId": "54dcdc571730aae4f1434e34",
				"profile": {
					"name": "Emma Clarke",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Magna cupidatat ea anim mollit."
			},
			{
				"commentId": "54dcdc585ea78943be8f24b4",
				"profile": {
					"name": "Saunders Rasmussen",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Ea id excepteur aliqua mollit occaecat occaecat non esse."
			},
			{
				"commentId": "54dcdc5859de299944f1d824",
				"profile": {
					"name": "Langley Ruiz",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Occaecat consectetur ullamco aliqua non nisi pariatur voluptate Lorem et labore."
			},
			{
				"commentId": "54dcdc5872c90cb94d0fb076",
				"profile": {
					"name": "Hunter Hewitt",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Cillum et deserunt aliquip nisi sint magna ipsum commodo mollit."
			},
			{
				"commentId": "54dcdc58005e677cabef96f7",
				"profile": {
					"name": "Pate Hughes",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Non proident officia eiusmod esse culpa Lorem eu nulla."
			}
		]
	},
	{
		"postId": 6,
		"backgroundImgUrl": "http://goo.gl/jMSO58",
		"profile": {
			"image": "http://i.imgur.com/WDFrSmC.png",
			"name": "Cuño Chileno"
		},
		"location": "Chillán",
		"timestamp": "1396621153000",
		"text": "En el trekking para a la cabaña 25!",
		"totalStars": 0,
		"totalComments": 2,
		"link": "",
		"comments": [
			{
				"commentId": "54dcdc8c50f536dcbc46e0f3",
				"profile": {
					"name": "Katheryn Delaney",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Exercitation anim fugiat et aute magna quis ut occaecat laborum laboris consequat."
			},
			{
				"commentId": "54dcdc8c0096a278cd71e7e9",
				"profile": {
					"name": "Albert Mccall",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Fugiat irure reprehenderit exercitation sit in."
			}
		]
	},
	{
		"postId": 3,
		"backgroundImgUrl": "http://goo.gl/wgyCuG",
		"profile": {
			"image": "http://i.imgur.com/dC0xDm4.png",
			"name": "Coneja Dientuda"
		},
		"location": "Lago Todos los Santos",
		"timestamp": "1396621153000",
		"text": "Hola mundo",
		"totalStars": 2,
		"totalComments": 5,
		"link": "",
		"comments": [
			{
				"commentId": "54dcdcb2c9ce59a3739bf2b8",
				"profile": {
					"name": "Rutledge Hess",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Eu excepteur fugiat eiusmod cillum mollit id amet veniam consectetur."
			},
			{
				"commentId": "54dcdcb2979eab13b249495c",
				"profile": {
					"name": "Matthews Paul",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Id anim proident incididunt veniam consequat elit in."
			},
			{
				"commentId": "54dcdcb21c773004865e2827",
				"profile": {
					"name": "Malinda Stephenson",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Aute do ad sint reprehenderit ex amet minim."
			},
			{
				"commentId": "54dcdcb2a6b4f1f2c0c255c6",
				"profile": {
					"name": "Mason Dotson",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Cupidatat cillum nisi fugiat et deserunt duis irure anim quis veniam labore reprehenderit voluptate."
			},
			{
				"commentId": "54dcdcb2553693de446b7bfd",
				"profile": {
					"name": "Smith Kennedy",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Aliqua nisi in sunt aute voluptate ipsum eu occaecat et ipsum reprehenderit fugiat magna."
			}
		]
	},
	{
		"postId": 4,
		"backgroundImgUrl": "http://goo.gl/oLqDKT",
		"profile": {
			"image": "http://goo.gl/syLmKX",
			"name": "Johnny Sins"
		},
		"location": "Pichilemu",
		"timestamp": "1396621153000",
		"text": "Respeto por la amistad y la pureza!",
		"totalStars": 2,
		"totalComments": 5,
		"link": "",
		"comments":
				[
					{
						"commentId": "54dcdcc7f6817605e39a23f5",
						"profile": {
							"name": "Edwards Wade",
							"image": "http://placehold.it/48x48"
						},
						"timestamp": "1396621153000",
						"text": "Fugiat cillum nulla incididunt deserunt non reprehenderit duis. Fugiat cillum nulla incididunt deserunt non reprehenderit duis. Fugiat cillum nulla incididunt deserunt non reprehenderit duis. Fugiat cillum nulla incididunt deserunt non reprehenderit duis."
					},
					{
						"commentId": "54dcdcc78fe1a1a3ae5e983f",
						"profile": {
							"name": "Mari Foley",
							"image": "http://placehold.it/48x48"
						},
						"timestamp": "1396621153000",
						"text": "Deserunt enim nulla amet consectetur veniam esse anim pariatur do ipsum."
					},
					{
						"commentId": "54dcdcc763061eaa213e6a47",
						"profile": {
							"name": "Moore Fowler",
							"image": "http://placehold.it/48x48"
						},
						"timestamp": "1396621153000",
						"text": "Sit aute duis consequat ullamco dolor aliquip ex."
					},
					{
						"commentId": "54dcdcc7e9df7d45c50af211",
						"profile": {
							"name": "Acevedo Lamb",
							"image": "http://placehold.it/48x48"
						},
						"timestamp": "1396621153000",
						"text": "Excepteur tempor adipisicing non pariatur ullamco laborum proident nulla laboris velit qui."
					},
					{
						"commentId": "54dcdcc7a3d636452f2c02dc",
						"profile": {
							"name": "Osborn Leblanc",
							"image": "http://placehold.it/48x48"
						},
						"timestamp": "1396621153000",
						"text": "Exercitation esse proident eu deserunt commodo commodo elit aliquip esse aliqua incididunt."
					}
				]
	},
	{
		"postId": 5,
		"backgroundImgUrl": "http://goo.gl/6G0uaG",
		"profile": {
			"image": "http://goo.gl/gZQkgx",
			"name": "Feastancia Pérez"
		},
		"location": "Arica",
		"timestamp": "1396621153000",
		"text": "Ahí con las chiquillas bailando en el agüita jijiji",
		"totalStars": 0,
		"totalComments": 2,
		"link": "",
		"comments": [
			{
				"commentId": "54dcdcdb381a10041925f2c9",
				"profile": {
					"name": "Marva Hensley",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Occaecat qui fugiat qui mollit nisi sit culpa ea non."
			},
			{
				"commentId": "54dcdcdbb48dd730650e1f10",
				"profile": {
					"name": "Alta Duke",
					"image": "http://placehold.it/48x48"
				},
				"timestamp": "1396621153000",
				"text": "Cupidatat officia consectetur fugiat reprehenderit aliquip exercitation mollit amet ullamco dolor ipsum."
			}
		]
	}
];